...
function phonetic(word) {
  ...
  for (var i = 0, l = FIRST_PREPROCESSING.length; i < l; i++) {
    var pattern = FIRST_PREPROCESSING[i][0];
    var replacement = FIRST_PREPROCESSING[i][1];
  ...
  for (var i = 0, l = SECOND_PREPROCESSING.length; i < l; i++) {
	var pattern = SECOND_PREPROCESSING[i][0];
    var replacement = SECOND_PREPROCESSING[i][1];
    code = code.replace(pattern, replacement);
  }
  ...
  // First endings
  for (var i = 0, l = FIRST_ENDINGS.length; i < l; i++) {
	var pattern = FIRST_ENDINGS[i][0];
    var replacement = FIRST_ENDINGS[i][1];
    code = code.replace(pattern, replacement);
  }
  ...
  // Second endings
  for (var i = 0, l = SECOND_ENDINGS.length; i < l; i++) {
	var pattern = SECOND_ENDINGS[i][0];
    var replacement = SECOND_ENDINGS[i][1];
    code = code.replace(pattern, replacement);
  }
